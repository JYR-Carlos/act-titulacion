...
> Para verificar a ojo si una prueba de `demo_vivo.py` reconoció bien la seña:
...
## Traducción en pantalla de `demo_vivo.py`

`demo_vivo.py` ahora traduce estas 10 glosas al español **solo para lo que se
...
`ETIQUETAS_ES` en `demo_vivo.py` (y viceversa) — no hay una fuente única
