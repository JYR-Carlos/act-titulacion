...
script de extracción actual (`scripts/extraer_keypoints.py`) agrega columnas `hand`
...
Meta del corpus: 10 señas dinámicas cerradas (ver `data/catalogos/Glosas_LSCh_Mappeadas.csv`),
...
- **`scripts/extraer_keypoints.py`** — extracción de keypoints con MediaPipe Tasks
...
- Integrar `scripts/extraer_keypoints.py` + `keypoint_normalizer.py` en un solo flujo que
