...
> `arquitectura_LSCh-MR.md` y `DECISION_PREPROCESAMIENTO.md` (no reabrir
...
- **Demo final: solo cámara** (PC + webcam). El Meta Quest 3 **no** es necesario
  ...
  Sentis sobre PC con webcam**; `scripts/demo_vivo.py` es el orquestador de referencia en
...
- `python scripts/generar_vectores_dorados.py --verificar` comprueba que el
...
Keypoints re-extraídos con `scripts/extraer_lote.py`; 483 de los 500 con detección.
...
> glosas de `data/catalogos/Glosas_LSCh_Mappeadas.csv`, que siguen siendo el vocabulario
...
> es el campo **2**. En un corpus grabado aquí con `scripts/grabar_corpus.py --senante s01`
...
|---|---|---|---|---|
| 1 | Precisión algorítmica | ≥ 85% | `scripts/evaluar_modelo.py` | ✅ cumple (ver tabla de arriba) |
| 2 | Latencia end-to-end | ≤ 500 ms | `scripts/demo_vivo.py` · `integracion/unity/` | ⚠️ medida, no cerrada |
| 3 | Rendimiento gráfico | ≥ 72 FPS | `integracion/unity/FrameRateMonitor.cs` | ❌ conflicto de alcance |
| 4 | Task success rate | ≥ 80% | `scripts/calcular_tsr.py` + `data/tsr/` | ❌ falta ejecutar sesiones |
| — | *(previa)* Límites del `RestStateDetector` | — | `scripts/evaluar_segmentacion.py` | ❌ falta etiquetar |
...
- **En LSA64 la mano solo se detecta en ~75% de los frames** y 405 de las 483
  ...
  `scripts/evaluar_segmentacion.py` imprime la tasa de detección justamente para poder
...
- **Pruebas con usuarios** (TSR ≥80% en ≥10 pruebas). El guion del escenario, la
  planilla y el cálculo ya están (`data/tsr/`, `scripts/calcular_tsr.py`); falta
  ejecutar las sesiones.
- **Ground truth de los límites de seña.** `scripts/evaluar_segmentacion.py` mide
...
> **Si algún día se graba corpus propio:** usar `scripts/grabar_corpus.py --senante s01`
...
scripts/extraer_keypoints.py         CLI extracción (webcam/video)
scripts/extraer_lote.py              CLI extracción por lotes (datasets de referencia)
scripts/grabar_corpus.py             CLI grabación del corpus (--senante)
scripts/build_dataset.py             CLI crudo -> dataset normalizado (+ gate de 50 muestras)
scripts/entrenar.py                  CLI entrenamiento (--cv, --cv-grupos, --solo-cv)
scripts/exportar_onnx.py             CLI exportación a ONNX
scripts/demo_vivo.py                 orquestador de referencia CU-01/CU-03 (latencia e2e, tecla r)
scripts/diagnostico_captura.py       CLI diagnóstico de captura (luz, tasa de detección)
scripts/generar_vectores_dorados.py  vectores de prueba (Capa 2) para el port a C#
scripts/generar_secuencia_dorada.py  secuencia de prueba (Capa 1+2) para el port a C#
scripts/evaluar_modelo.py            CLI métrica 1: accuracy + matriz de confusión
scripts/evaluar_segmentacion.py      CLI prueba previa: límites del RestStateDetector
scripts/calcular_tsr.py              CLI métrica 4: task success rate
scripts/reproducir_metricas.py       las cuatro evaluaciones del informe de una vez
scripts/verificar_artefactos.py      coherencia modelo.onnx / labels.json / dorados (CI)
scripts/_raiz.py                     pone la raíz del repo en sys.path
docs/CONTEXTO_PROYECTO.md    el diseño: casos de uso, secciones y modelo de datos
docs/arquitectura_LSCh-MR.md documento de arquitectura (el que se audita)
docs/METRICAS_MVP.md         cómo se mide cada una de las 4 métricas del MVP
docs/INTEGRACION_UNITY.md    contrato entre repos + spec del port
docs/CONTEXTO_UNITY.md       contexto del subsistema de presentación (Capa 4)
docs/DECISION_PREPROCESAMIENTO.md por qué el preprocesamiento es propio
docs/GLOSARIO_SENAS_MODELO.md las 10 glosas del modelo y su traducción
docs/PROTOCOLO_TSR.md        guion del escenario de ventanilla (métrica 4)
...
tools/                       assembler_repo.py + el prompt de evaluación
tests/                       124 tests unitarios
data/catalogos/              Glosas_LSCh_Mappeadas.csv, lsa64_10_annotations.csv
data/tsr/                    planilla del TSR (métrica 4)
data/external/lsa64/         corpus proxy LSA64 (no versionado)
data/raw/lsa64_10.csv        keypoints del proxy (no versionado)
data/processed/*.npz         datasets normalizados (los dos del informe, versionados)
outputs/                     modelo, labels y reportes
```
...
sostenida con la cámara real del demo (`python scripts/demo_vivo.py --fuente 0`, que deja
el reporte solo).
...
1. **Etiquetar el ground truth** de límites de seña (40-60 muestras bastan) y
   correr `scripts/evaluar_segmentacion.py`.
