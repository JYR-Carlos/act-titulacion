...
Del lado Python, `python scripts/generar_vectores_dorados.py --verificar` comprueba que el
...
El corpus del MVP es LSA64 y se extrajo con `scripts/extraer_lote.py`, que crea el
provider así:

```python
# scripts/extraer_lote.py:77
...
> tiene `running_mode="video"` por defecto, y solo `scripts/extraer_lote.py` lo
> sobrescribía. Los otros tres usos iban en `VIDEO`: `scripts/demo_vivo.py` (que alimenta
> a `modelo.onnx`, o sea que la propia demo de referencia tenía el skew),
> `DatasetRecorder` y `scripts/extraer_keypoints.py --modo webcam` (que escriben corpus).
...
Medido con `scripts/generar_secuencia_dorada.py` sobre el mismo vídeo del corpus
...
7. **No pasar la imagen en espejo.** El corpus se extrajo sin voltear el frame
   (`scripts/extraer_lote.py` usa `FuenteVideo(str(vid))`, con `espejo=False`). Voltear
   ...
   > `scripts/demo_vivo.py` volteaba por defecto **y pasaba ese mismo frame volteado al
...
8. **La mano dominante se elige por frame, sin memoria.** `modo_manos="dominante"`
   ...
   pipeline, pero es exactamente lo que hizo `scripts/build_dataset.py` al construir el
...
Del lado Python, `python scripts/generar_secuencia_dorada.py --verificar` re-extrae el
