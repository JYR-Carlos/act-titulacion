...
|---|---------|--------|-----------------|--------|
| 1 | Precisión algorítmica | ≥ 85% | `scripts/evaluar_modelo.py` | ✅ **0.911 ± 0.051** (k-fold por señante) |
| 2 | Latencia end-to-end | ≤ 500 ms | `scripts/demo_vivo.py` (PC) · `MetricsRecorder.cs` (Unity) | ⚠️ medida, no cerrada |
| 3 | Rendimiento gráfico | ≥ 72 FPS | `FrameRateMonitor.cs` | ❌ requiere runtime Unity |
| 4 | Task Success Rate | ≥ 80% | `scripts/calcular_tsr.py` | ❌ pendiente de sesión |
| — | *(previa)* Límites del RestStateDetector | — | `scripts/evaluar_segmentacion.py` | ❌ pendiente de etiquetado |
...
las 10 glosas de `data/catalogos/Glosas_LSCh_Mappeadas.csv`, que siguen siendo el vocabulario
...
señante `001`. **Ojo**: en un corpus grabado con `scripts/grabar_corpus.py --senante s01`
...
```bash
copy data\tsr\planilla_tsr_plantilla.csv data\tsr\planilla_tsr.csv
# ... ejecutar las pruebas rellenando la planilla ...
python scripts/calcular_tsr.py --planilla data\tsr\planilla_tsr.csv
```

Guion completo del escenario de ventanilla simulada, criterio de éxito y
checklist de sesión: **`PROTOCOLO_TSR.md`**. Léelo entero antes de la
...
python scripts/evaluar_modelo.py --fuente cv                   # métrica 1
python scripts/calcular_tsr.py                                 # métrica 4 (si hay planilla)
```
