...
Ver la tabla y su explicación en `ESTADO_ACTUAL.md`.

Uso:
    # Recomendado para el informe (k-fold por señante, ya calculado):
    python evaluar_modelo.py --fuente cv --cv-json outputs/reports/cv_metrics_dominante_senante.json
    ...
    python evaluar_modelo.py --fuente modelo --backend keras --dataset ...
...
def _predecir_onnx(onnx_path: Path, X: np.ndarray) -> np.ndarray:
    ...
    if not onnx_path.exists():
        raise FileNotFoundError(
            f"No existe {onnx_path}. Expórtalo con: python exportar_onnx.py")
...
def _predecir_keras(keras_path: Path, X: np.ndarray) -> np.ndarray:
    if not keras_path.exists():
        raise FileNotFoundError(
            f"No existe {keras_path}. Entrénalo con: python entrenar.py")
...
def _evaluar_modelo(args) -> dict:
    ...
    if not ds.exists():
        raise FileNotFoundError(
            f"No existe el dataset {ds}. Constrúyelo con: python build_dataset.py")
...
def _evaluar_desde_cv(args) -> dict:
    ...
    if not ruta.exists():
        raise FileNotFoundError(
            ...
            f"    python entrenar.py --dataset {args.dataset} --cv-grupos 2 --solo-cv\n"
