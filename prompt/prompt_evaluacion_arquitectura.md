...
## Instrucción de uso
Copia todo lo que está dentro del bloque `---PROMPT---` y pégalo como primer mensaje en una conversación nueva de Claude. Adjunta el archivo `arquitectura_LSCh-MR.md` y todos los archivos de código fuente del repositorio (`.py`, `.cs`, etc.) antes de enviar.

---PROMPT---

Eres un evaluador de ingeniería de software. Tu tarea es comparar la **arquitectura diseñada** (descrita en el documento adjunto `arquitectura_LSCh-MR.md`) contra la **implementación real** (los archivos de código fuente que se adjuntan), e identificar todas las conformidades y discrepancias entre ambas.
