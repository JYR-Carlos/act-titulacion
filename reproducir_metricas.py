...
Uso:
    python reproducir_metricas.py                # corrida completa (TARDA)
    python reproducir_metricas.py --dry-run      # solo imprime los comandos
    python reproducir_metricas.py --solo-tablas  # tablas desde los JSON en disco
...
def _comandos(ds_dominante: Path, ds_ambas: Path, campo_senante: str) -> list[tuple[str, list[str]]]:
    ...
    return [
        ("dominante / estratificado",
         py + ["entrenar.py", "--dataset", str(ds_dominante), "--cv", "--solo-cv"]),
        ("dominante / por señante",
         py + ["entrenar.py", "--dataset", str(ds_dominante),
               "--cv-grupos", campo_senante, "--solo-cv"]),
        ("ambas / estratificado",
         py + ["entrenar.py", "--dataset", str(ds_ambas), "--cv", "--solo-cv"]),
        ("ambas / por señante",
         py + ["entrenar.py", "--dataset", str(ds_ambas),
               "--cv-grupos", campo_senante, "--solo-cv"]),
        ("métrica 1 sobre las predicciones out-of-fold",
         py + ["evaluar_modelo.py", "--fuente", "cv"]),
        ("métrica 1 sobre el ONNX exportado (20% retenido)",
         py + ["evaluar_modelo.py", "--fuente", "modelo",
