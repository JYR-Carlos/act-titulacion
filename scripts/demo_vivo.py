...
Uso:
    ...
    python scripts/demo_vivo.py --realce          # escena oscura (ver lsch_mr/realce_luz.py)
...
# vocabulario LSCh, ver docs/CONTEXTO_UNITY.md sección 2) y eso no cambia aquí; esto
...
# tabla de docs/GLOSARIO_SENAS_MODELO.md.
ETIQUETAS_ES = {
    "Accept": "Aceptar",
    "Appear": "Aparecer",
    "Call": "Llamar",
    ...
    if cap["n_frames"]:
        ...
        if not historial and cap["tasa_deteccion_mano"] < 0.7:
            print("[demo] no se segmento ninguna sena y la mano se ve en menos "
                  ...
                  + " y contrasta con: python scripts/diagnostico_captura.py --etiqueta <cond>")
...
def main() -> int:
    ...
    # Decisión cerrada: docs/INTEGRACION_UNITY.md sección 7.
    #
    ...
    # Ver docs/INTEGRACION_UNITY.md sección 7.6, trampa 7.
    ...
    try:
        ...
        raise SystemExit(
            ...
            "  Para ver qué está viendo la cámara: python scripts/diagnostico_captura.py")
