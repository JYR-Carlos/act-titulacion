...
que describe INTEGRACION_UNITY.md sección 2, y es silencioso por naturaleza.

Este script lo convierte en un fallo ruidoso. Corre en CI y no necesita corpus,
cámara ni TensorFlow.

Uso:
    python verificar_artefactos.py
    python verificar_artefactos.py --sin-onnx    # solo los JSON (sin onnxruntime)
"""
...
def main() -> int:
    ...
    if args.sin_onnx:
        ...
        v.comprobar(abs(float(probs.sum()) - 1.0) < 1e-4,
                    ...
                    f"suma {float(probs.sum()):.4f}; INTEGRACION_UNITY.md §6.5 "
    ...
    dorados = Path("integracion/vectores_dorados.json")
    if v.comprobar(dorados.exists(), "integracion/vectores_dorados.json existe"):
        ...
        v.comprobar(sha1_real in texto,
                    ...
                    "al reentrenar: regenera con `python generar_vectores_dorados.py` "
                    "y avisa al equipo de Unity (INTEGRACION_UNITY.md §5)")
